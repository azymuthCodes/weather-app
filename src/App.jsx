...
import { WiDaySunny,WiCloud,WiDaySunnyOvercast, WiRain, WiDayCloudy, WiDayWindy, WiDayLightning,} from "react-icons/wi";
...
function App() {
  ...
  backgrounds.set('mild','background: linear-gradient(90deg, rgba(0,165,215,1) 0%, rgba(54,132,168,1) 39%, rgba(156,173,177,1) 100%)');
  backgrounds.set('snow','linear-gradient(90deg, rgba(240,240,240,1) 0%, rgba(244,244,244,1) 39%, rgba(249,249,249,1) 100%)');
  const[background,setBackground] = useState(backgrounds.get('bright'));
  ...
  function setWeatherData(cityText,temperatureText,humidityText,wind_speedText){
    ...
    if(temperatureText.toString().length<3){
      setCurrentWidth("60px");
    }
    else{
      setCurrentWidth("80px");
    }
  ...
  return(
    <div className="container" style={{'background':background}}>
      <div className="search-box">
        <form className="location-input" autoComplete="off" onSubmit={(event)=>{event.preventDefault()}}>
      ...
      <div className="temperature-box">
        <div className="temperature">
          ...
          <div className = "temp-info" style={{'width':currentWidth}}>
